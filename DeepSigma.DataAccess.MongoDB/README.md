...
|---|---|---|
| `GetByIdAsync<T>(db, collection, id, [cancellationToken])` | `T?` | `T : IMongoDocument`. Returns `null` if not found. |
| `FindAsync<T>(db, collection, [filter], [sort], skip, take, [cancellationToken])` | `IReadOnlyList<T>` | Pages with `Skip` + `Limit`; default page is `[0, 100)`. |
| `CountAsync<T>(db, collection, [filter], [cancellationToken])` | `long` | Counts documents matching the filter (empty filter counts all). |
| `InsertAsync<T>(db, collection, document, [cancellationToken])` | `Task` | Single insert. |
| `InsertAsync<T>(db, collection, documents, [cancellationToken])` | `Task` | Bulk insert via `InsertManyAsync`. |
| `ReplaceAsync<T>(db, collection, document, upsert, [cancellationToken])` | `bool` | `T : IMongoDocument`. Returns `true` on a successful modify or upsert. |
| `DeleteAsync<T>(db, collection, id, [cancellationToken])` | `bool` | `T : IMongoDocument`. Returns `true` if a document was deleted. |
| `DeleteManyAsync<T>(db, collection, filter, [cancellationToken])` | `bool` | Returns `true` if at least one document was deleted. |
| `CreateIndexAsync<T>(db, collection, keys, [options], [cancellationToken])` | `string` | Returns the created index name. |
...
services.AddDeepSigmaMongoDb("mongodb://localhost:27017/");
```

Registers `MongoDbApi` as a singleton. The underlying `MongoClient` is reused across the application as recommended by the driver.
...
Pairs with the `Microsoft.Extensions.Diagnostics.HealthChecks` system. The check runs `{ ping: 1 }` against the `admin` database — no application database or collection required.

```csharp
services.AddHealthChecks()
    .AddDeepSigmaMongoDb("mongodb://localhost:27017/", tags: new[] { "readiness" });

app.MapHealthChecks("/health");
```

Optional arguments (all named): `name` (default `"deepsigma_mongodb"`), `failureStatus` (default `Unhealthy`), `tags`, `timeout` (recommended: `TimeSpan.FromSeconds(5)`).

A throwaway `MongoClient` is created per check — the driver pools connections internally, so this is fine for periodic probes; for very high-frequency probes (sub-second) you may want to roll your own check that reuses the singleton client.
