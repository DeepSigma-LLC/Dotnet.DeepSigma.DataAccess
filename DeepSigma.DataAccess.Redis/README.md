...
// Set
await cache.SetCacheDataAsync(
    key: "user:42",
    value: new { Name = "Ada", Email = "ada@example.com" },
    expirationTime: DateTimeOffset.UtcNow.AddMinutes(30));

// Get
var user = await cache.GetCacheDataAsync<UserDto>("user:42");

// Remove
await cache.RemoveCacheDataAsync("user:42");
```
...
- **TTL calculation.** `SetCacheDataAsync` computes the TTL as `expirationTime.DateTime.Subtract(DateTime.Now)`. Pass a `DateTimeOffset` in a comparable kind (UTC vs local) to avoid surprises — and pass a value in the future, otherwise the TTL is negative and the key may behave unexpectedly.
